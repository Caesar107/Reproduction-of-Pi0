...
def describe(tree):
    if isinstance(tree, np.ndarray):
        return {"type": "ndarray", "shape": tree.shape, "dtype": tree.dtype}
    if isinstance(tree, dict):
        return {k: describe(v) for k, v in tree.items()}
    return {"type": type(tree).__name__, "value": tree}

client = WebsocketClientPolicy(host="127.0.0.1", port=8000)
obs = aloha_policy.make_aloha_example()
action = client.infer(obs)
pprint.pp(describe(action))
PY
```


```bash
uv run scripts/train_pytorch.py pi0_aloha_pen_uncap --exp_name=my_pen_run --overwrite
```