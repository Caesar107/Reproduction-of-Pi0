...
@dataclasses.dataclass(frozen=True)
class LeRobotCustomRobotDataConfig(DataConfigFactory):
    """
    Custom robot data config for training with your own dataset.
    
    This config handles data from caesar/my_robot_demo dataset which contains:
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Import the custom policy transforms
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        # Map dataset observation keys to intermediate keys
                        "observation.state": "observation.state",
                        "observation.end_effector_state": "observation.end_effector_state",
        ...
        # Data transforms: convert to model input format
        data_transforms = _transforms.Group(
        ...
        # Apply delta action transform if needed
        # Your actions have 7 dimensions - apply delta to all except gripper (last dim)
        if self.use_delta_actions:
            delta_action_mask = _transforms.make_bool_mask(6, -1)  # Delta for first 6, absolute for last 1
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
        ...
        return dataclasses.replace(
            ...
            data_transforms=data_transforms,
            model_transforms=model_transforms,
            prompt_from_task=True,  # Use task field as prompt
            action_sequence_keys=("action",),  # Our dataset uses "action" not "actions"
        )
...
_CONFIGS = [
    ...
    #
    # Custom robot fine-tuning configs
    #
    TrainConfig(
